...
/* 
void grppi_tbb(grppi::dynamic_execution & ex) {
    ex.pipeline(
	    []() mutable -> optional<spb::Item> {
            spb::Item item;
            if(!spb::source_op(item)) {
...
grppi::dynamic_execution execution_mode(grppi::queue_mode queue_mode, bool set_ordering, int queue_size, int tokens) {
  ...
  if ("tbb" == opt) {
    if(set_ordering) {
        auto aux = grppi::parallel_execution_tbb(spb::nthreads, true);//spb::nthreads, true
        //aux.enable_ordering();
        aux.set_queue_attributes(queue_size, queue_mode, tokens);
        return aux;
    }
    if(!set_ordering) {
        auto aux = grppi::parallel_execution_tbb(spb::nthreads, false);//spb::nthreads, false
        aux.set_queue_attributes(queue_size, queue_mode, tokens); 
        //aux.disable_ordering();
        return aux;
    }
...
void run() {
    auto exec = execution_mode(grppi::queue_mode::blocking, false, 1, spb::nthreads);
